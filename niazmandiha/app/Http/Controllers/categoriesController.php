...
class categoriesController extends Controller
{
    ...
    public function categories($what)
    {
        switch ($what) {
            ...
            case 'car' :
                $agahi=DB::table('posts')->join('vehicles','posts.id','=','vehicles.post_id')->where('vehicles.car',1)->get();
                return view('home',compact('agahi'));
            case 'car-part' :
                $agahi=DB::table('posts')->join('vehicles','posts.id','=','vehicles.post_id')->where('vehicles.car_part',1)->get();
                return view('home',compact('agahi'));
            case 'motor' :
                $agahi=DB::table('posts')->join('vehicles','posts.id','=','vehicles.post_id')->where('vehicles.motor',1)->get();
                return view('home',compact('agahi'));
            case 'boat' :
                $agahi=DB::table('posts')->join('vehicles','posts.id','=','vehicles.post_id')->where('vehicles.boat',1)->get();
                return view('home',compact('agahi'));
            case 'heavy_car' :
